...
SubMap::SubMap(const SubMap::Config& config) {
    ...
    for (int i = 0; i < config_.num_tiles; ++i) {
        tiles_.sharp.at(i).reset(new CloudData::CloudT());
        tiles_.flat.at(i).reset(new CloudData::CloudT());
    }
...
SubMap::LocalMap SubMap::GetLocalMap(
    ...
    // Convert query position to tile index
...
void SubMap::FormGlobalMap(CloudData::CloudT::Ptr &cloud_ptr) {
    cloud_ptr->clear();

    for (int i = 0; i < tiles_.flat.size(); ++i) {
        *cloud_ptr += *tiles_.flat[i];
        *cloud_ptr += *tiles_.sharp[i];
    }
}
...
SubMap::Index SubMap::GetTileIndex(const CloudData::PointT &point) {
    Eigen::Vector3d t{
...
bool SubMap::IsValidIndex(const SubMap::Index& index) {
    if ((0 <= index.x) && (index.x < config_.num_tiles_x) && 
...
SubMap::LocalMap SubMap::GetLocalMap(
    ...
    local_map.sharp.reset(new CloudData::CloudT());
    local_map.flat.reset(new CloudData::CloudT());
...
bool SubMap::RegisterFeaturePoints(
    ...
    CloudData::CloudT::Ptr target(new CloudData::CloudT());

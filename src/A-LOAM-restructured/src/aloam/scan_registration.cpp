...
ScanRegistration::ScanRegistration(void) {
    ...
    // Set ALOAM scan registration params
...
bool ScanRegistration::InitFilters(const YAML::Node& config_node) {
    surf_less_flat_filter_ptr_ = std::make_unique<pcl::VoxelGrid<CloudData::PointT>>();
...
bool ScanRegistration::Update(
    ...
    // Filter input pointclouds
    CloudData::CloudT filtered_cloud;
    FilterByRange(*input_cloud.cloud_ptr, filtered_cloud);

    // Sort pointcloud by scans
    output_cloud.reset(new CloudData::CloudT);
...
bool ScanRegistration::FilterByRange(
    const CloudData::CloudT &input_cloud, 
    CloudData::CloudT &output_cloud) {
    // Drop all NaNs
...
float ScanRegistration::GetCurvature(const CloudData::CloudT &cloud, int point_index) {
...
bool ScanRegistration::SortPointCloudByScan(const CloudData::CloudT &input_cloud, CloudData::CloudT &output_cloud) {
    ...
    std::vector<CloudData::CloudT> scan(config_.num_scans);
    for (int i = 0; i < input_size; i++)
    {
        ...
        // Set intensity for motion compensation
    ...
    // Set start & end indices for each scan
...
bool ScanRegistration::PickInNeighborhood(const CloudData::CloudT &cloud, const int point_index, const float thresh) {
    // Forward
...
bool ScanRegistration::GetFeaturePoints(
    ...
    CloudData::CloudT::Ptr &surf_flat,
    CloudData::CloudT::Ptr &surf_less_flat) {
    corner_sharp.reset(new CloudData::CloudT());
    corner_less_sharp.reset(new CloudData::CloudT());
    surf_flat.reset(new CloudData::CloudT());
    surf_less_flat.reset(new CloudData::CloudT());

    for (int i = 0; i < config_.num_scans; ++i) {
        if(index_.scan.end[i] - index_.scan.start[i] < config_.num_sectors)
            continue;

        CloudData::CloudT::Ptr surf_less_flat_scan_ptr(new CloudData::CloudT);
        ...
        CloudData::CloudT surf_less_flat_scan_downsampled;

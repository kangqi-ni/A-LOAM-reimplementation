...
std::vector<float> read_lidar_data(const std::string lidar_data_path)
{
    ...
    // Point to the front of the file for ;ater reading
    lidar_data_file.seekg(0, std::ios::beg);

    std::vector<float> lidar_data_buffer(num_elements);
    // Read a number of float elements into the vector
...
int main(int argc, char** argv)
{
    ...
    // Set a rotation to transform NEU to image frame convention(kitti's convention)
    ...
    ros::Rate r(10.0 / publish_delay); // 10HZ frequency
    while (std::getline(timestamp_file, line) && ros::ok())
    {
        ...
        // Transform the ground truth pose to kitti's frame convention
        ...
        // Convert pcl pointcloud format to ros pointcloud format
        ...
        // Convert opencv image format to ros image format
        ...
        // Write all the data to a ros bag
        if (to_bag)
        {
